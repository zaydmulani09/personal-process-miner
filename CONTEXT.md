...
| P27 | UIA-Native Automation Engine: coordinate-based automation replaced with UIA-native element targeting; pyautogui now only used for keypress and scroll; UI tree re-read before every step; one-action-at-a-time execution loop | complete |
...
- **P27 UIA-native engine**: `accessibility.py` fully rewritten with `get_window_tree` (replaces `get_screen_tree`), `execute_action` (UIA-native: focus_window/click/type/menu_select; pyautogui only for keypress+scroll), `_find_control` fallback chain (automation_id → name+type → name → type+index). `get_screen_tree` kept as backward-compat shim. `nl_planner.parse_instruction` now prompts AI for new action schema (not coordinates); `execute_instruction` re-reads UI tree before every step with one corrective retry on element-not-found. `vision_replay._execute_step` converts legacy step format to action schema via `_step_to_action`. `NLBuilder.tsx` "Run Now" calls `execute_nl_instruction` IPC directly (no ReplayControls modal). `get_automation_steps` IPC detects nl_builder script_type and returns JSON steps directly.
